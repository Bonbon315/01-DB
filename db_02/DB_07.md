...
```
여기서 get 메소드는 반드시 하나의 반환값이 있어야한다. 없거나 많으면 오류가 뜬다. 이 경우엔 country가 USA인 Director가 하나도 없기때문에 DoesNotExist 에러가 떳다.
```
...
```python
d = Director.objects.get(name='Joseph Kosinski')
d.country = 'USA'
d.save()
...
```
여기서 get 메소드는 반드시 하나의 반환값이 있어야한다. 없거나 많으면 오류가 뜬다. 이 경우엔 country가 KOR인 Director가 7명이 있기 때문에 오류가 발생했다.
```
...
```
filter는 리스트의 형태로 반환해 주는 것이 가장 큰 차이점이다.
```
